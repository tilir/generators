...
template <size_t N> class all_spanning_MS {
  Graph<N> &T;
  set<size_t> D;
  vector<size_t> NUM;
  vector<bool> USED;
...
public:
  ...
  void do_all_spanning(function<void(Graph<N>)> fcallback, size_t min,
                       size_t jmin) {
    fcallback(T);
...
template <template <size_t> typename F> int test_all() {
  size_t count;
